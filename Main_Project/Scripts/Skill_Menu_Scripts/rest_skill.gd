...
func _ready():
	...
	var off_combat_xp = ceil(1.5 * (Player.MAX_HP + Player.MAX_MP + Player.MAX_EN))
	
	var combat_restore = (10 * Skill.Rest_LVL) + Player.END
	var combat_xp = ceil(2.5 * Player.MAX_EN)
