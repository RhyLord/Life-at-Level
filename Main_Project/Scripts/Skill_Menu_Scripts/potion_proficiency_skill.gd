...
func _ready():
	...
	Nxt_Level_txt.text = "Progress to Next Level : " + str(Skill.Potion_Proficiency_XP) + " / " + str(Skill.Potion_Proficiency_NXT_XP)
	...
	Off_combat_desc.text = "Passive: Gain EXP when crafting or using potions. EXP Gain = " + str(50 + (Player.DEX *  + Skill.Potion_Proficiency_LVL)) + ")."
